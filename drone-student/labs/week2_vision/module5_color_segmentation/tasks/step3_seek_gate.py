...
def update(drone):
    ...
    largest_gate = neo_lab.largest_cyan_gate(image, MIN_AREA)

    if (largest_gate is None):
        print("Need to search again.")
        drone.flight.send_pcmd(0, 0, SEARCH_YAW, 0)
        return False
    x, y, w, h = cv2.boundingRect(largest_gate)
    
    column = x + w/2 
    error = (column - COL_CENTER) / COL_CENTER                          
    yaw = uav_utils.clamp(-error * MAX_YAW, -MAX_YAW, MAX_YAW)
   
    if abs(column - COL_CENTER) < CENTER_TOL:
    ...
    if w >=  TARGET_WIDTH:
        drone.flight.stop()
        print("Gate is in ideal proximity!")
        _done = True
