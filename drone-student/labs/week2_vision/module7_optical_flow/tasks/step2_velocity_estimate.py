...
def update(drone):
    global _prev_gray, _prev_pts, _timer, _interval, _frame, _done, _estimated_velocity, _true_velocity
    if _done:
        return True
    ##################################
    #### START PUT CODE HERE #########
    ...
    if _frame % SKIP == 0:
        ...
        if _prev_pts is None or _prev_gray is None or len(_prev_pts) < MIN_PTS:
            pts = cv2.goodFeaturesToTrack(gray, **FEATURE_PARAMS)
            if pts is not None:
                _prev_pts = pts
                _prev_gray = gray
        else:
            new_pts, status, error = cv2.calcOpticalFlowPyrLK(_prev_gray, gray, _prev_pts, None, **LK_PARAMS)
            
            keep = status.flatten() == 1
            valid_new = new_pts[keep].reshape(-1, 2)
            valid_old = _prev_pts[keep].reshape(-1, 2)

            if len(valid_new) >= MIN_PTS:
                displacements = valid_new - valid_old
                _last_mag = float(np.mean(np.sqrt(displacements[:, 0] ** 2 + displacements[:, 1] ** 2)))
                _prev_pts = valid_new.reshape(-1, 1, 2)
                current_height = max(neo_lab.height(drone), 0.1)
                ground_width = 2.0 * current_height * HFOV_TAN
                meters_per_pixel = ground_width / IMAGE_WIDTH

                _estimated_velocity = float(-(_last_mag * meters_per_pixel) / max(_interval, 0.001))

                vx, vy, vz = drone.physics.get_linear_velocity()
                _true_velocity = (float(vx), float(vz))
            else:
                _prev_pts = None
    ...
    if _timer >= RUN_TIME:
        drone.flight.stop() 
        print(f"{_estimated_velocity} m/s is the estimated velocity, while the true one is {_true_velocity} m/s.")
        _done = True
