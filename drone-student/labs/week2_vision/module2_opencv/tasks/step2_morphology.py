...
def update(drone):
    ...
    image_erosion = cv2.erode(mask, kernel, iterations = 1)
    image_dilation = cv2.dilate(image_erosion, kernel, iterations = 1)
    opened_image = cv2.morphologyEx(image_dilation, cv2.MORPH_OPEN, kernel)
    new_white_fraction = np.count_nonzero(opened_image) / mask.size * 100
    old_white_fraction = np.count_nonzero(mask) / mask.size * 100

    
    _timer += drone.get_delta_time()
    if _timer >= HOVER_TIME:
        print(f"Before, {old_white_fraction} % of the pixels were white, but now {new_white_fraction} % are.")
        _done = True
