#!/usr/bin/env python3
"""
UAV Neo Connection Diagnostics

Tests the networking path between WSL2/Linux and the UAVNeo Simulator.
    ...
    try:
        result = subprocess.run(
            [
                "powershell.exe",
                "-Command",
                'Get-NetFirewallRule -DisplayName "WSL2 UAVNeo Simulator" '
...
def main():
    print("=== UAV Neo Connection Diagnostics ===")
    print()
    ...
    if not issues:
        ...
        print("and try running: drone sim demo.py")
    else:
